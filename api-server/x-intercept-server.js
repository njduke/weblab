...
export const router_x_intercept = express.Router();

const app_id = 'YU83PL-H7P4JR4JVK';

// GET ENTRYPOINT
router_x_intercept.get('/', function(req, res, next) {
    console.log('x-intercept calculation request: ' + req.ip);
    next();
})

router_x_intercept.use('/', async function (req, res, next) {
    try {
        const math_query = encode_uri(req.query.function);
        const wolfram_result = await wolfram_x_intercept(math_query);
        const result_encoded = encode_uri(wolfram_result);
        console.log(`Calculated X-Intercept: ${wolfram_result}`);
...
async function wolfram_x_intercept(function_string) {
    try {
        ...
        const request_uri = `http://api.wolframalpha.com/v1/result?i=x+intercept+of+${function_string}&appid=${app_id}`;
